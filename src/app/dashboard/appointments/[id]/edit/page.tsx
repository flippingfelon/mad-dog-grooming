...
import type { Owner, Dog, AppointmentWithDetails, AppointmentStatus } from "@/lib/types";
...
export default function EditAppointmentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    Promise.all([
    ...
    ]).then(([apptData, ownersData]) => {
      const a = apptData.appointment as AppointmentWithDetails;
      if (!a) {
      ...
      return fetch(`/api/dogs?owner_id=${a.owner_id}`).then((r) => r.json());
    }).then((dogsData) => {
      if (dogsData) setDogs(dogsData.dogs ?? []);
      setLoading(false);
    });
  }, [id, router]);

  useEffect(() => {
    if (!form.owner_id || loading) return;
    fetch(`/api/dogs?owner_id=${form.owner_id}`)
      .then((r) => r.json())
      .then((d) => setDogs(d.dogs ?? []));
  }, [form.owner_id, loading]);
  ...
  return (
    <div className="mx-auto max-w-xl space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-brown">Edit Appointment</h1>
        <div className="flex gap-2">
          <button onClick={deleteAppt} className="rounded-lg border border-red/30 px-3 py-1.5 text-sm font-medium text-red hover:bg-red/10 transition-colors">
            Delete
          </button>
      ...
      {overlapWarning && (
        <p className="rounded-lg bg-gold/15 px-4 py-2.5 text-sm font-medium text-gold-dark">
          ⚠ This appointment overlaps with another booking. Changes saved.
        </p>
      )}
